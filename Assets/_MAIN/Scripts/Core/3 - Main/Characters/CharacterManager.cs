...
namespace CHARACTERS
{
    public class CharacterManager : MonoBehaviour
    {
        ...
        public Character GetCharacter(string characterName, bool createIfDoesNotExist = false)
        {
            ...
            throw new KeyNotFoundException($"No character found with name '{characterName}'.");
        }

        public bool HasCharacter(string characterName) => characters.ContainsKey(characterName.ToLower());

        public Character CreateCharacter(string characterName, bool revealAtCreation = false)
        {
            if(characters.ContainsKey(characterName.ToLower()))
            {
                Debug.LogWarning($"A character called '{characterName}' already exists. Character not created.");
                return null;
            }
            ...
            if (revealAtCreation) character.Show();
        ...
        private Character CreateCharacterFromInfo(CHARACTER_INFO info)
        {
            ...
            {
                case Character.CharacterType.Text: { return new Character_Text(info.name, config); }
                case Character.CharacterType.Sprite:
                case Character.CharacterType.SpriteSheet: { return new Character_Sprite(info.name, config, info.prefab, info.rootCharacterFolder); }
                case Character.CharacterType.Live2D: { return new Character_Live2D(info.name, config, info.prefab, info.rootCharacterFolder); }
                default: { return null; }
            }
        }
        ...
        {
            List<Character> sortedCharacters = sortedCharacters = characterNames.Select(name => GetCharacter(name)).Where(character => character != null).ToList();
